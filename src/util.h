 ...
 *
 * $Id: util.h,v 1.2 2002/06/07 03:45:54 vadimp Exp $
 */

void fatal_error ( const char* message, const int use_errno );
__inline int lnum (const char *c);
char* strndup ( const char* s, size_t n );
char* unify_word ( char* s );
char* convert_to_logical ( char* dst, const char* src );
char* convert_to_physical ( char* dst, const char* src );

/*
 * Copies from src to dst only characters equal to ch while leaving the rest of
 * characters in dst intact.
 */
char* strccpy ( char* dst, const char* src, char ch );

/*
 * Recodes string from one Cyrillic codepage to another
 * Returns dst
 */
